...
class VanillaGradientDescent(BaseDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        """
        # Создаем объект LearningRate при первом вызове, если он еще не существует
        if not hasattr(self, 'lr'):
            self.lr = LearningRate(lambda_=self.lambda_)

        # Вычисляем длину шага η_k
        eta_k = self.lr()
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        # Выбираем индексы для батча и соответсвующие объекты
        n_samples = x.shape[0]
...
class MomentumDescent(VanillaGradientDescent):
    ...
    def __init__(self, dimension: int, lambda_: float = 1e-3, loss_function: LossFunction = LossFunction.MSE):
        ...
        """
        super().__init__(dimension, lambda_, loss_function)
        self.alpha: float = 0.9 # коэффициент инерции
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        # Обновляем значение момента h_{k+1} = alpha * h_k + eta_k * gradient
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        # Вычисляем eta_k с использованием LearningRate
        if not hasattr(self, 'lr'):
            self.lr = LearningRate(lambda_=self.lambda_)
